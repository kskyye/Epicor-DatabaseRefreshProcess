...
#Name:             Epicor Environment Refresh Partial Automation Script v1.3
#Date:             2018 08 27
...
Function Show-Menu
{
     ...
     (
        ...
        Write-Host -for Yellow "(for detailed documentation on this process go to the Epicor Teams Channel & select 'Epicor Technical Support)"
        ...
        #Check to see if pre-refresh tasks have been completed for an environment.  If so, display option to run post refresh tasks
        If ($targetDescription -ne "") 
        {
            Write-Host -for Yellow "" 
            Write-Host -for Green "!!Pre-refresh tasks for one or more environments have been completed!!  Use option below *AFTER* Data Team confirms refresh package ran succesfully."
            Write-host -for DarkYellow "R: for *POST* DB-Refresh Tasks for $targetDescription"
        }
        Write-Host -for DarkYellow " "
        Write-Host -for DarkYellow "(Press 'Q' at any time to Quit.)"
}
...
####BEGIN RUNNING *POST* DATABASE-REFRESH TASKS`
                ...
                #Set flag to indicate that the entire refresh process has been completed.
...
####BEGIN RUNNING *PRE* DATABASE-REFRESH TASKS IF AN ENVIRONMENT REFRESH WAS COMPLETED
    ...
    If ($refreshComplete -eq $true)
    {
        Write-Host -for Yellow "Refresh of Environment is complete. Remaining tasks:"
        Write-Host -for DarkYellow "1. Set the manager password for all lower environments back to default (password after refresh is the manager password for PROD)."
        Write-Host -for DarkYellow "2. Send out Maintenance Completed notification email."
        Write-Host -for Green "Refresh of $targetDescription is complete."

        $refreshComplete = $false
        $targetDescription = ""
        Pause
    } 
    ElseIf ($refreshComplete -eq $false)
    {
        ...
        Write-Host -for Cyan "Pre Database Refresh Tasks Complete - Please inform the participating member of the Data Team that they can now run their refresh package."
        Write-Host -for Cyan "After the Data Team confirms the refresh has been completed click to continue; then,"
        Write-Host -for Cyan "once you're returned to the main menu SELECT 'R' to run the post refresh tasks for the current environment: ***$targetDescription***"
        
        #Reset TargetDescription variable
